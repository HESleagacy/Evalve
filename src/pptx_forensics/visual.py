...
def _layout_objects(objects: Iterable[dict[str, Any]]) -> list[tuple[dict[str, Any], Box]]:
...
def _add_visual_regions(
    ...
    for slide in sorted(deck.slides, key=lambda item: (item.get("number", 0), item.get("id", ""))):
        slide_id = slide["id"]
        positioned = _layout_objects(objects_by_slide.get(slide_id, []))
...
def _slide_native_facts(
    ...
    slide_id = slide["id"]
    positioned = _layout_objects(objects)
    ...
    for region in _empty_regions(area_boxes):
        facts.append(
            _fact(
                "whitespace_region",
                slide_id,
    ...
    font_values = [size for item in text_items for size in _font_sizes(item)]
    ...
    facts.append(
        _fact(
            ...
            slide_id,
            {
                ...
                "histogram": [{"size_pt": size, "count": histogram[size]} for size in sorted(histogram)],
...
def add_native_visual_evidence(
    ...
    _add_visual_regions(deck, objects_by_slide)
